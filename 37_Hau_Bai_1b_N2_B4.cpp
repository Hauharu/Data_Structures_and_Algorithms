...
void countingSort_37_Hau_Bai_1b_N2_B4(int a[], int b[], int n, int k)
{
...
//tao so ngau nhien
void nhapMangRandom_37_Hau_Bai_1b_N2_B4(int arr[], int& n, int& k) {
	srand(time(NULL));
	...
	{
		arr[i] = rand() % (10 - 0 + 1) + 0;// to so ngau nhien
		if (k < arr[i])// so sanh gia tri moi nhap voi bien k
			k = arr[i];// gan gia tri a[i] cho k
	}
}
// xuat danh sach
void xuatMang_37_Hau_Bai_1b_N2_B4(int a[], int n)
{
...
int main() {
	...
	nhapMangRandom_37_Hau_Bai_1b_N2_B4(data, n, k);
	countingSort_37_Hau_Bai_1b_N2_B4(data, dataSorted, n, k);
	cout << "Mang truoc khi sap xep: \n";
	xuatMang_37_Hau_Bai_1b_N2_B4(data, n);
	cout << "\nMang sau khi sap xep: \n";
	xuatMang_37_Hau_Bai_1b_N2_B4(dataSorted, n);
	cout << endl;
	system("pause");
	return 0;
}
